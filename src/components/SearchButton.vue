...
onMounted(() => {
  ...
  window.addEventListener('mousemove', mouseSearchBtnListener)
...
onUnmounted(() => {
  ...
  window.removeEventListener('mousemove', mouseSearchBtnListener)
...
const mouseSearchBtnListener = (e) => {
...
<template>
  <div
    ref="searchBtnRef"
    class="fixed opacity-0 transition top-0 left-0 z-50 pointer-events-none min-w-[2.5rem] min-h-[2.5rem] rounded-full bg-white flex justify-center items-center"
    :class="searchInputShow ? 'px-2' : 'px-0'"
  >
    ...
    <input
      ref="searchInputRef"
      class="select-none outline-none border-0 transition-all"
